...
let mongoDB = new MongoClient({ useUnifiedTopology: true })
admin.initializeApp({
        ...
        server.route({
          ...
          handler: async (request, h) => {
            ...
            let urlComponents = getURLComponents(clientData[0])
                ...
                let existing = await propertiesDB.find({ domain: urlComponents.hostname }).toArray();
        ...
        server.route({
          ...
          handler: async function  (request, h) {
            ...
            if (uid) {
              ...
              let existing = await propertiesDB.find({ domain: body.domain }).toArray();
              console.log(existing)
              if (existing.length > 0) {
                return h.response({ success: false, error: "domain in use" }).code(401);
                ...
                return h.response({ success: true, id: id }).code(200);
